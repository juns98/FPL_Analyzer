...
function Team() {
  ...
  const answerHeaders = [["Rank", "Name", "Win", "Draw", "Lose", "Points", "+", "-", "Diff"]];

  const getAnswer = async () => {
    const serverUrl = process.env.REACT_APP_SERVER_ADDRESS;
    const teamInfo = await axios.get(`http://${serverUrl}/team-info`);
    const teamStanding = await axios.get(`http://${serverUrl}/team-standing`);
    console.log(teamStanding.data);
    setAnswers(teamStanding.data);
  };
    ...
    if (questionIndex === 0) {
      return (
        <AnswerData>
          ...
          <AnswerComponent>{data.team.tla}</AnswerComponent>
          <AnswerComponent>{data.won}</AnswerComponent>
          <AnswerComponent>{data.draw}</AnswerComponent>
          <AnswerComponent>{data.lost}</AnswerComponent>
          <AnswerComponent>{data.points}</AnswerComponent>
          <AnswerComponent>{data.goalsFor}</AnswerComponent>
          <AnswerComponent>{data.goalsAgainst}</AnswerComponent>
